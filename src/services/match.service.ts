        ...
        return {
          ...
          home_team: {
            ...
            players: isHome ? [...match.home_team.players].sort((a, b) => {
              ...
              return a.captain ? -1 : 1; // true (captain) comes first, false (non-captain) comes last
            }).map((player: any) => ({
              id: player.player?.id,
  ...
  async publicMatchList(req: any, res: any, status?: MatchStatus) {
    ...
    try {
      ...
      const queryBuilder = matchesRepo
        ...
        .andWhere("matches.deletedAt IS NULL")
        .andWhere("matches.deletedBy IS NULL")
        .andWhere("away_players.deletedAt IS NULL")
        .andWhere("home_players.deletedAt IS NULL")
        .andWhere("matches.deletedBy IS NULL")
